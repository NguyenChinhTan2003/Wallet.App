...
namespace Wallet.App.BillDetails
{
    ...
    public abstract class BillDetailsAppServiceBase : AppAppService
    {
        ...
        protected IRepository<Wallet.App.PaymentTypeCategories.PaymentTypeCategory, int> _paymentTypeCategoryRepository;
        protected IRepository<Volo.Abp.Identity.IdentityUser, Guid> _identityUserRepository;

        public BillDetailsAppServiceBase(IBillDetailRepository billDetailRepository, BillDetailManager billDetailManager, IDistributedCache<BillDetailDownloadTokenCacheItem, string> downloadTokenCache, IRepository<Wallet.App.PaymentTypeCategories.PaymentTypeCategory, int> paymentTypeCategoryRepository, IRepository<Volo.Abp.Identity.IdentityUser, Guid> identityUserRepository)
        {
        ...
        public virtual async Task<List<TotalCategoryDto>> GetCategory()
        {
            return (await _billDetailRepository.GetCategory()).Select(x => ObjectMapper.Map<TotalCategoryViewModel, TotalCategoryDto>(x)).ToList();
        }
        ...
        public virtual async Task<PagedResultDto<LookupDto<int>>> GetPaymentTypeCategoryLookupAsync(LookupRequestDto input)
        {
            ...
            var lookupData = await query.PageBy(input.SkipCount, input.MaxResultCount).ToDynamicListAsync<Wallet.App.PaymentTypeCategories.PaymentTypeCategory>();
            var totalCount = query.Count();
            return new PagedResultDto<LookupDto<int>>
            {
                TotalCount = totalCount,
                Items = ObjectMapper.Map<List<Wallet.App.PaymentTypeCategories.PaymentTypeCategory>, List<LookupDto<int>>>(lookupData)
            };
        }
        ...
        public virtual async Task<Wallet.App.Shared.DownloadTokenResultDto> GetDownloadTokenAsync()
        {
            ...
            return new Wallet.App.Shared.DownloadTokenResultDto
            {
                Token = token
            };
        }
